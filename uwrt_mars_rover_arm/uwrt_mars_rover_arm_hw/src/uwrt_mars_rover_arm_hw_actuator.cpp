...
bool ArmActuatorInterface::configure_transmission() {
  ...
  if (!actuator_velocity_state_handle || !joint_velocity_state_hanlde || !actuator_position_state_handle ||
      !joint_position_state_handle || !actuator_velocity_command_handle || !joint_velocity_command_handle) {
    return false;
  }

  // configure both state & command transmissions
  command_transmission->configure({*joint_velocity_command_handle}, {*actuator_velocity_command_handle});
  state_transmission->configure({*joint_velocity_state_hanlde, *joint_position_state_handle},
                                {*actuator_velocity_state_handle, *actuator_position_state_handle});

  return true;
}

ArmActuatorInterface::ArmActuatorInterface() : actuator_logger(rclcpp::get_logger("ArmActuatorInterface")) {}
...
std::vector<StateInterface> ArmActuatorInterface::export_state_interfaces() {
  ...
  state_interfaces_list.emplace_back(
      StateInterface(info_.joints.at(0).name, hardware_interface::HW_IF_POSITION, &actuator_states.position));
  state_interfaces_list.emplace_back(
      StateInterface(info_.joints.at(0).name, hardware_interface::HW_IF_VELOCITY, &actuator_states.velocity));

  state_interfaces_list.emplace_back(
      StateInterface(info_.joints.at(0).name, ArmActuatorInterface::HW_IF_IQ_CURRENT, &actuator_states.iq_current));
...
std::vector<CommandInterface> ArmActuatorInterface::export_command_interfaces() {
  ...
  command_interface_list.emplace_back(
      CommandInterface(info_.joints.at(0).name, hardware_interface::HW_IF_POSITION, &joint_commands.velocity));

  return command_interface_list;
}

// we will need this once we have multiple ways of controlling
return_type ArmActuatorInterface::prepare_command_mode_switch(const std::vector<std::string>& start_interfaces,
...
return_type ArmActuatorInterface::read() {
  ...
  RCLCPP_INFO_STREAM(actuator_logger,
...
return_type ArmActuatorInterface::write() {
  ...
  actuator_commands.velocity = joint_to_actuator_reduction * joint_commands.velocity;  // transmission
...
CallbackReturn ArmActuatorInterface::on_init(const HardwareInfo& hardware_info) {
  if (hardware_interface::ActuatorInterface::on_init(hardware_info) != CallbackReturn::SUCCESS) {
    RCLCPP_FATAL_STREAM(rclcpp::get_logger("UWRTMarsRoverArmActuator"), "FAILED TO PARSE ARM URDF ...");
  ...
  if (info_.joints.size() != NUM_JOINTS) {
    RCLCPP_FATAL_STREAM(actuator_logger, "'" << info_.name.c_str() << "' has " << info_.joints.size()
                                             << " joints. Expected: " << NUM_JOINTS << "'");
    return CallbackReturn::ERROR;
  }

  const hardware_interface::ComponentInfo& joint = info_.joints.at(0);  // alias for easy use - only one joint

  if (joint.state_interfaces.size() != NUM_STATE_INTERFACES) {
    RCLCPP_FATAL_STREAM(actuator_logger, "Joint '" << joint.name.c_str() << "' has " << joint.state_interfaces.size()
  ...
  if (joint.command_interfaces.size() != NUM_COMMAND_INTERFACES) {
    RCLCPP_FATAL_STREAM(actuator_logger, "Joint '" << joint.name.c_str() << "' has " << joint.command_interfaces.size()
  ...
  for (const hardware_interface::InterfaceInfo& state_interfaces : joint.state_interfaces) {
    if (!(state_interfaces.name == hardware_interface::HW_IF_POSITION ||
      ...
      RCLCPP_FATAL_STREAM(actuator_logger, "Joint '" << joint.name.c_str() << "'has " << state_interfaces.name.c_str()
  ...
  // lol literally only one interface ... could use if statement ... but for sake of scalability and coherency lets use
  // a for loop
  for (const hardware_interface::InterfaceInfo& command_interface : joint.command_interfaces) {
    if (!(command_interface.name == hardware_interface::HW_IF_VELOCITY)) {
      RCLCPP_FATAL_STREAM(actuator_logger, "Joint '" << joint.name.c_str() << "'has " << command_interface.name.c_str()
...
PLUGINLIB_EXPORT_CLASS(uwrt_mars_rover_arm_hw::ArmActuatorInterface, hardware_interface::ActuatorInterface)
