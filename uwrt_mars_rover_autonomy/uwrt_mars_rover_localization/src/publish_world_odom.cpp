...
class TransformWorldOdom {
 ...
 public:
  TransformWorldOdom(std::string name, ros::NodeHandle &nh) : node_name(std::move(name)), tf_listener(tf_buffer) {
    target_frame = uwrt_mars_rover_utils::getParam<std::string>(nh, node_name, "target_frame", "map");

    std::string odom_topic =
        uwrt_mars_rover_utils::getParam<std::string>(nh, node_name, "odom_topic", "vectornav/Odom");
  ...
  void transformOdom(const nav_msgs::OdometryConstPtr &odom) {
    ...
    try {
      ...
      ROS_ERROR_STREAM_NAMED(node_name, "Received an exception trying to transform pose from "
                                            << odom->header.frame_id << " to " << target_frame << ": " << ex.what());
    }
  }

  void transformTwist(const geometry_msgs::Twist &origin_twist, geometry_msgs::Twist &target_twist,
