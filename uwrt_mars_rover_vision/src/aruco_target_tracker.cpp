...
void TargetTracker::camInfoCallback(const sensor_msgs::msg::CameraInfo::SharedPtr info) {
  ...
  if (intrinsic_calib_matrix_.size().height == 0) {
    ...
    RCLCPP_DEBUG_STREAM(this->get_logger(), "fx: " << info->k[0] << " | cx: " << info->k[2] << " | fy: " << info->k[4]
                                                   << " | cy: " << info->k[5]);
    int param_num = 0;
    ...
    intrinsic_calib_matrix_ = (cv::Mat_<float>(3, 3) << info->k[0], 0, info->k[2], 0, info->k[4], info->k[5], 0, 0, 1);
  }
}
