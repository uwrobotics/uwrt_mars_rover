...
TargetTracker::TargetTracker(const rclcpp::NodeOptions& options) : Node("target_tracker", options) 
{
  ...
  display_marker_pose_ = this->get_parameter("display_marker_pose").get_parameter_value().get<bool>();
  std::string aruco_dict = this->get_parameter("aruco_dict").get_parameter_value().get<std::string>();
  RCLCPP_DEBUG_STREAM(this->get_logger(), "Marker Len: " << aruco_marker_len_);

  camera_info_sub_ = create_subscription<sensor_msgs::msg::CameraInfo>("/zed2/zed_node/left/camera_info", 10, std::bind(&TargetTracker::camInfoCallback, this, std::placeholders::_1));

  rmw_qos_profile_t custom_qos = rmw_qos_profile_default;
  camera_sub_ = image_transport::create_subscription(this, "/zed2/zed_node/left/image_rect_color", std::bind(&TargetTracker::imageCallback, this, std::placeholders::_1), "raw", custom_qos);
  
  aruco_pose_pub_ = create_publisher<geometry_msgs::msg::PoseStamped>("/aruco_pose", 10);
  
  // do aruco setup stuff
  obj_points_.ptr<cv::Vec3f>(0)[0] = cv::Vec3f(-aruco_marker_len_/2.f, aruco_marker_len_/2.f, 0);
  obj_points_.ptr<cv::Vec3f>(0)[1] = cv::Vec3f(aruco_marker_len_/2.f, aruco_marker_len_/2.f, 0);
  obj_points_.ptr<cv::Vec3f>(0)[2] = cv::Vec3f(aruco_marker_len_/2.f, -aruco_marker_len_/2.f, 0);
  obj_points_.ptr<cv::Vec3f>(0)[3] = cv::Vec3f(-aruco_marker_len_/2.f, -aruco_marker_len_/2.f, 0);
