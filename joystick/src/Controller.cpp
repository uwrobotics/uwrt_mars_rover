...

{
  ...
  arm_publisher = nh_.advertise<uwrt_arm_msgs::UWRTArmTwist>("/arm_cartesian_controller/twist_arm_cmds", 1);
...
void TeleopTurtle::joyCallback(const sensor_msgs::Joy::ConstPtr& joy) {
  ...
  uwrt_arm_msgs::UWRTArmTwist twist_arm_cmds;
  ...
  //   z_axis_ = (-joy->axes[LT_] + 1) / 2.0;  // up
  // }
  // if (joy->axes[LT_] == 1 && joy->axes[RT_] != 1) {
  //   z_axis_ = -(-joy->axes[RT_] + 1) / 2.0;  // down
  // }
  ...
  // std::vector<double> arm_vec = {turntable_vel, z_axis_, elbow_vel, claw_rotate, claw_up_down};
