...
void execute(const drive_to_gps::gps_goalGoalConstPtr &goal, Server *as) {
  ...
  sensor_msgs::NavSatFixPtr gps_goal(
      new sensor_msgs::NavSatFix(goal->gps_goal));
  bool end = false;
  ros::Rate rate(1);
  while (calculate_distance(gps_goal, curr_gps) > MAX_ERROR && !end &&
         ros::ok()) {
    ...
    ROS_INFO(
        "Heading between current heading and goal is %d and direction is %d",
        heading_diff, multiplier);
...
int main(int argc, char **argv) {
  ...
  Server server(n, "calculate_twist", boost::bind(&execute, _1, &server),
                false);
  ros::Subscriber sub_curr_heading =
      nq.subscribe("/heading/curr", 1, update_current_heading);
  ros::Subscriber sub_curr_gps =
      nq.subscribe("/gps/fix", 1, update_current_gps);
  server.start();
