...
class ArmDifferentialSystemInterface : public hardware_interface::SystemInterface {
 ...
 private:
  using ActuatorCommandsData_t = std::map<const std::string, DifferentialTransmissionData::ActuatorData::CommandData>;
  using ActuatorStatesData_t = std::map<const std::string, DifferentialTransmissionData::ActuatorData::StateData>;
  using JointStatesData_t = std::map<const std::string, DifferentialTransmissionData::JointData::StateData>;
  using JointCommandsData_t = std::map<const std::string, DifferentialTransmissionData::JointData::CommandData>;

 protected:
  // ARM Actuator URDF defines these numbers
  inline static constexpr std::size_t NUM_JOINTS{2};  // 2 joints & 2 actuators since it is a differential transmission
  ...
  std::map<const std::string, transmission_interface::ActuatorHandle> actuator_velocity_state_handle{};
  std::map<const std::string, transmission_interface::JointHandle> joint_velocity_state_hanlde{};
  std::map<const std::string, transmission_interface::ActuatorHandle> actuator_position_state_handle{};
  ...
  std::map<const std::string, transmission_interface::ActuatorHandle> actuator_velocity_command_handle{};
