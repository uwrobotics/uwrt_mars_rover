...
{
    UWRTCartesianController::UWRTCartesianController() : m_cart_command_sub(nullptr), m_ik_solver(nullptr), m_rt_buffer(nullptr) {}
        ...
        const rclcpp_lifecycle::State & /*previous_state*/)
    {
        ...
        {
            RCLCPP_ERROR(get_node()->get_logger(), ("Failed to get KDL chain from tree: " + m_root_name + " --> " + m_tip_name).c_str());
            RCLCPP_ERROR(get_node()->get_logger(), tree_info.str().c_str());
            return CallbackReturn::ERROR;
        }

        RCLCPP_INFO(get_node()->get_logger(), ("Successfully got KDL chain from tree: " + m_root_name + " --> " + m_tip_name).c_str());
        ...
        m_cart_command_sub = get_node()->create_subscription<TwistStamped>(
            ...
            { m_rt_buffer.writeFromNonRT(msg); }); // younes todo any chance msg could be null?
        ...
        const rclcpp_lifecycle::State & /*previous_state*/)
    {
        ...
        if (command_interfaces_.size() != m_joint_names.size() || state_interfaces_.size() != m_joint_names.size())
        {
            RCLCPP_ERROR(node_->get_logger(), "Expected %zu velocity command interfaces, got %zu. Expected %zu position state interfaces, got %zu.", m_joint_names.size(),
                         command_interfaces_.size(), m_joint_names.size(), state_interfaces_.size());
            return CallbackReturn::ERROR;
    ...
    controller_interface::return_type UWRTCartesianController::update(const rclcpp::Time &time, const rclcpp::Duration & /*period*/)
    {
        ...
        if(!end_effector_cart_vel || !*end_effector_cart_vel || (time - (*end_effector_cart_vel)->header.stamp) > rclcpp::Duration(5, 0))
        {
    ...
    bool UWRTCartesianController::parse_params()
    {
        ...
        const std::unordered_map<std::string *, const std::string> string_params = {{&m_cart_command_topic, "end_effector_velocity_topic"},
        ...
        {   
            *(string_param.first) = node_->get_parameter(string_param.second).as_string();
            ...
            {
                RCLCPP_ERROR(get_node()->get_logger(), (string_param.second + " parameter was empty").c_str());
                return false;
            }
        }
